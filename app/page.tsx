  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        {section === "Stock master" ? <StockMaster /> : section === "POS" ? <FinancialControl /> : <div className="content">
...
function FinancialControl() {
  ...
  const [tab, setTab] = useState("Dashboard"); const [message, setMessage] = useState("Loading financial controls…");
  ...
  async function loadFinance() { try { const response = await fetch("/api/finance"); const data = await response.json(); if (!response.ok) throw new Error(data.error); setSuppliers(data.suppliers); setInvoices(data.invoices); setTransactions(data.transactions); setInvoiceForm((form) => ({ ...form, supplierId: form.supplierId || data.suppliers[0]?.id || 0 })); setMessage("Financial ledger online"); } catch { setMessage("Financial database is preparing. Refresh shortly."); } }
  ...
  const calendarStart = new Date(); calendarStart.setDate(1); const daysInMonth = new Date(calendarStart.getFullYear(), calendarStart.getMonth() + 1, 0).getDate(); const offset = (calendarStart.getDay() + 6) % 7;
  ...
  return <div className="content finance-control">
    ...
    <div className="finance-tabs">{["Dashboard", "POS documents", "Supplier accounts", "Supplier calendar"].map((name) => <button key={name} className={tab === name ? "active" : ""} onClick={() => setTab(name)}>{name}</button>)}<span>{message}</span></div>
    ...
    {tab === "Supplier calendar" && <div className="finance-columns calendar-layout"><section className="panel calendar-panel"><div className="panel-head"><div><small>PAYMENT CALENDAR</small><h2>{calendarStart.toLocaleDateString("en-ZA", { month: "long", year: "numeric" })}</h2></div><span>Due dates calculated from delivery</span></div><div className="calendar-week">{["MON", "TUE", "WED", "THU", "FRI", "SAT", "SUN"].map((day) => <span key={day}>{day}</span>)}</div><div className="calendar-grid">{Array.from({ length: offset }).map((_, i) => <div key={`blank-${i}`} />)}{Array.from({ length: daysInMonth }).map((_, index) => { const day = index + 1; const date = isoDate(new Date(calendarStart.getFullYear(), calendarStart.getMonth(), day)); const items = invoices.filter((invoice) => invoice.dueDate === date); return <div key={day} className={`${date === today ? "today" : ""} ${date === tomorrow ? "tomorrow" : ""}`}><b>{day}</b>{items.map((invoice) => <span key={invoice.id} title={`${invoice.supplierName} ${invoice.invoiceNumber}`}>{invoice.invoiceNumber}<small>{money(invoice.balance)}</small></span>)}</div>; })}</div></section><SupplierInvoiceList invoices={invoices} /></div>}

    {tab === "Dashboard" && <div className="finance-columns"><SupplierInvoiceList invoices={invoices} /><TransactionRegister transactions={transactions} /></div>}
  </div>;
}
