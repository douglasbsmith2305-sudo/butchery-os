...
function FinancialControl({ initialTab = "Dashboard" }: { initialTab?: string }) {
  ...
  return <div className="content finance-control">
    <div className="eyebrow">POS / FINANCIAL CONTROL</div><div className="page-heading"><div><h1>Sales, accounts and payments in one ledger.</h1><p>Control customer accounts, notes, returns, payouts and supplier payment deadlines.</p></div><span className="status-pill">{dueTomorrow.length} DUE TOMORROW</span></div>
    ...
    <div className="finance-tabs">{["Accounts overview", "Supplier calendar", "Supplier accounts", "POS documents"].map((name) => <button key={name} className={tab === name ? "active" : ""} onClick={() => setTab(name)}>{name}</button>)}<span>{message}</span></div>
