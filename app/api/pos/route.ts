...
export async function GET() {
  ...
  const [products, accounts, sales, movements] = await Promise.all([
    ...
    env.DB.prepare("SELECT id, account_number AS accountNumber, name, credit_limit AS creditLimit, balance, status FROM customer_accounts WHERE status='ACTIVE' ORDER BY account_number").all(),
...
export async function POST(request: Request) {
  ...
  if (payload.action === "SALE") {
    ...
    if (account && account.balance + total > account.creditLimit) return Response.json({ error: "This sale exceeds the account credit limit" }, { status: 409 });
    ...
    if (account) {
      const purchase = await env.DB.prepare("INSERT INTO customer_purchases (account_id, invoice_number, purchase_date, total, balance, status) VALUES (?, ?, ?, ?, ?, 'OPEN')").bind(account.id, saleNumber, date, total, total).run();
      const purchaseId = Number(purchase.meta.last_row_id);
      await env.DB.batch([env.DB.prepare("UPDATE customer_accounts SET balance=balance+? WHERE id=?").bind(total, account.id), ...lines.map(line => env.DB.prepare("INSERT INTO customer_purchase_items (purchase_id, product_name, quantity, unit, unit_price, line_total) VALUES (?, ?, ?, ?, ?, ?)").bind(purchaseId, line.name, line.quantity, line.unit, line.unitPrice, line.quantity * line.unitPrice))]);
    }
    return Response.json({ ok: true, saleNumber, total });
  }
