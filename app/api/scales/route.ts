...
export async function GET(request:Request) {
  ...
  if(deviceId&&(format==="csv"||format==="json")){
    ...
    return Response.json({device,job,generatedAt:new Date().toISOString(),items:mappings.results.map(item=>({productId:item.productId,plu:item.plu,sku:item.sku,barcodePrefix:item.barcodePrefix,labelName:item.labelName,department:item.department,unit:item.unit,pricePerKg:item.sellingPrice}))});
  }
  ...
  const [devices,mappings,jobs]=await Promise.all([
    env.DB.prepare("SELECT id,name,brand,connector_type AS connectorType,location,endpoint,database_table AS databaseTable,active,last_sync_at AS lastSyncAt,last_sync_status AS lastSyncStatus,created_at AS createdAt FROM scale_devices ORDER BY name").all(),
...
export async function POST(request:Request) {
  ...
  if(body.action==="device"){
    ...
    await env.DB.prepare("INSERT INTO scale_devices (name,brand,connector_type,location,endpoint,database_table,active,last_sync_status,created_at) VALUES (?,?,?,?,?,?,1,'NEVER',?) ON CONFLICT(name) DO UPDATE SET brand=excluded.brand,connector_type=excluded.connector_type,location=excluded.location,endpoint=excluded.endpoint,database_table=excluded.database_table,active=1").bind(name,String(body.brand??"Other"),String(body.connectorType??"CSV"),String(body.location??"Store"),String(body.endpoint??""),String(body.databaseTable??"plu_master"),now).run(); return Response.json({ok:true});
  }
  if(body.action==="mapping"){
    ...
    await env.DB.prepare("INSERT INTO scale_product_mappings (device_id,product_id,plu,barcode_prefix,label_name,enabled,updated_at) VALUES (?,?,?,?,?,1,?) ON CONFLICT(device_id,product_id) DO UPDATE SET plu=excluded.plu,barcode_prefix=excluded.barcode_prefix,label_name=excluded.label_name,enabled=1,updated_at=excluded.updated_at").bind(deviceId,productId,plu,String(body.barcodePrefix??"20"),String(body.labelName??product.name).slice(0,30),now).run(); return Response.json({ok:true});
  }
  if(body.action==="sync"){
