...
export async function GET() {
  ...
  const [products, wastes, counts, settings, sales, supplierInvoices, productSales, priceHistory, priceApprovals, monthWaste, monthVariance] = await Promise.all([
    env.DB.prepare("SELECT id,sku,barcode,name,department,unit,cost_price AS costPrice,selling_price AS sellingPrice,target_margin_percent AS targetMarginPercent,quantity FROM products ORDER BY name").all(),
