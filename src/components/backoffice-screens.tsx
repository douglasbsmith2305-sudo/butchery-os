...
const input = "h-11 w-full rounded-lg border border-[#303840] bg-[#0c0f12] px-3 text-sm text-[#edf2f5] outline-none focus:border-[#528cad] focus:ring-2 focus:ring-[#528cad]/15";
...
const subtleButton = "inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border border-[#35404a] bg-[#151a1e] px-3 text-xs font-semibold text-[#b9c4cb] transition hover:border-[#4d6574] hover:text-white disabled:opacity-40";
...
function Title({ eyebrow, title, copy, action }: { eyebrow: string; title: string; copy: string; action?: React.ReactNode }) {
  return <div className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
    <div><p className={`${label} mb-2 text-[#719bb2]`}>{eyebrow}</p><h1 className="text-2xl font-semibold tracking-tight sm:text-[30px]">{title}</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-[#818c95]">{copy}</p></div>{action}
  </div>;
}
...
export function BackOfficeDashboard() {
  ...
  return <>
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {controls.map(({ name, copy, href, icon: Icon, detail }) => <Link href={href} key={name} className={`${card} group p-5 transition hover:border-[#3f5664]`}>
        <div className="flex items-center justify-between"><span className="grid size-10 place-items-center rounded-lg border border-[#365161] bg-[#14232c] text-[#80bddc]"><Icon size={19}/></span><span className="text-[11px] text-[#6f7a84]">{detail}</span></div>
        <h2 className="mt-5 text-sm font-semibold group-hover:text-[#a9d6ec]">{name}</h2><p className="mt-2 text-xs leading-5 text-[#7d8891]">{copy}</p>
      </Link>)}
    </div>
  </>;
}
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[340px_1fr]">
      <div className={`${card} h-fit p-5`}>
        ...
        <div className="mt-3 grid gap-2">{(Object.keys(datasetLabels) as ImportDataset[]).map((key) => <button key={key} onClick={() => { setDataset(key); setPreview(null); setFilename(""); }} className={`rounded-lg border p-3 text-left text-sm ${dataset === key ? "border-[#4f7f99] bg-[#172630] text-[#a8d4ea]" : "border-[#293138] bg-[#0d1013] text-[#8f9aa3]"}`}>{datasetLabels[key]}</button>)}</div>
      ...
      <div className={`${card} overflow-hidden`}>
        <div className="border-b border-[#272e34] p-5">
          ...
          <label className="mt-4 flex min-h-36 cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-[#40505b] bg-[#0c0f12] p-6 text-center transition hover:border-[#63869a]">
            <Upload size={24} className="text-[#76b5d7]"/><span className="mt-3 text-sm font-semibold">{filename || `Choose ${datasetLabels[dataset]} CSV`}</span><span className="mt-1 text-xs text-[#707c85]">CSV only · data is previewed before import</span>
          ...
          <details className="mt-3"><summary className="cursor-pointer text-xs text-[#78b5d3]">Or paste CSV text</summary><textarea aria-label="Paste CSV text" className={`${input} mt-3 min-h-28 py-3 font-mono text-xs`} placeholder={csvTemplates[dataset]} onBlur={(event) => event.target.value.trim() && loadText(event.target.value, "pasted-data.csv")}/></details>
        </div>
        {preview ? <>
          <div className="flex flex-wrap items-center gap-2 border-b border-[#272e34] p-4">
...
export function ProductsSettingsScreen() {
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[1fr_370px]">
      <div className={`${card} overflow-hidden`}><div className="scrollbar overflow-x-auto"><table className="w-full min-w-[820px] text-left text-xs"><thead><tr className="border-b border-[#272e34] text-[#707c85]">
