...
const secondary = "inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-[#303840] bg-[#15191d] px-4 text-sm font-medium text-[#c2cbd1] transition hover:border-[#46515a]";

function Header({ eyebrow, title, copy, action }: { eyebrow: string; title: string; copy: string; action?: React.ReactNode }) {
  return <div className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className={`${label} mb-2 text-[#719bb2]`}>{eyebrow}</p><h1 className="text-2xl font-semibold tracking-tight sm:text-[30px]">{title}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#818c95]">{copy}</p></div>{action}</div>;
}
...
function Badge({ status }: { status: BatchStatus }) {
  const tone = status === "Raw" ? "border-[#31546a] bg-[#132735] text-[#79b9dc]" : status === "Part processed" ? "border-[#5f4820] bg-[#2c210d] text-[#e0aa4b]" : "border-[#28533e] bg-[#10291f] text-[#5fc78f]";
    ...
    <div className="grid gap-4 xl:grid-cols-[1.05fr_.95fr]">
      <div className={`${card} p-5 sm:p-7`}>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <Field name="Actual scale weight (kg)" hint="Stock quantity"><input className={`${input} border-[#426a80] font-mono text-lg font-semibold`} min=".001" step=".001" type="number" value={actualKg} onChange={(event) => { setActualKg(event.target.value); setReviewed(false); }} /></Field>
        ...
        <div className="mt-6 grid gap-3 rounded-lg border border-[#2b343b] bg-[#0c0f12] p-4 sm:grid-cols-2">
...
export function ProcessingScreen() {
  ...
  return <>
    ...
    <div className={`${card} overflow-hidden`}>
      <div className="grid gap-4 border-b border-[#272e34] p-5 md:grid-cols-3">
