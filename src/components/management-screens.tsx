...
const input = "h-11 rounded-lg border border-[#303840] bg-[#0c0f12] px-3 text-sm text-[#edf2f5] outline-none transition focus:border-[#528cad] focus:ring-2 focus:ring-[#528cad]/15";
const primary = "inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-[#dbe8ef] px-4 text-sm font-semibold text-[#10161a] transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-40";
...
function Header({ eyebrow, title, copy, children }: { eyebrow: string; title: string; copy: string; children?: React.ReactNode }) {
  return <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
    <div><p className={`${label} mb-2 text-[#719bb2]`}>{eyebrow}</p><h1 className="text-2xl font-semibold tracking-tight sm:text-[30px]">{title}</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-[#818c95]">{copy}</p></div>
    {children}
  </div>;
}

function RangePicker({ value, onChange }: { value: ManagementRange; onChange(value: ManagementRange): void }) {
  return <div className="inline-flex rounded-lg border border-[#2d343a] bg-[#0c0f12] p-1">
...
function Status({ children, tone = "green" }: { children: React.ReactNode; tone?: "green" | "amber" | "red" | "gray" | "blue" }) {
  const tones = {
    ...
    red: "border-[#613037] bg-[#2f1519] text-[#e87980]", gray: "border-[#353c42] bg-[#1a1e22] text-[#a0a8af]",
    ...
    <div className="grid gap-4 xl:grid-cols-[1.15fr_.85fr]">
      <div className={`${card} p-5`}>
        ...
        <div className="mt-6 space-y-4">{paymentTotals.map((item) => <div key={item.method}>
          ...
          <div className="h-2 overflow-hidden rounded-full bg-[#20262b]"><div className="h-full rounded-full bg-gradient-to-r from-[#37647c] to-[#71b3d4]" style={{ width: `${item.amount / maxPayment * 100}%` }}/></div>
        </div>)}</div>
        <div className="mt-6 grid grid-cols-2 gap-3 border-t border-[#272e34] pt-5">
          <div className="rounded-lg bg-[#0d1013] p-4"><p className={label}>Average sale</p><p className="mt-2 font-mono text-lg font-semibold">{zar.format(sales.length ? summary.revenue / sales.length : 0)}</p></div>
          <div className="rounded-lg bg-[#0d1013] p-4"><p className={label}>Stock integrity</p><p className={`mt-2 text-lg font-semibold ${reconciliation.reconciled ? "text-[#67d098]" : "text-[#e5b45c]"}`}>{reconciliation.reconciled ? "Reconciled" : `${reconciliation.varianceKg.toFixed(2)} kg variance`}</p></div>
        </div>
      </div>
      <div className={`${card} overflow-hidden`}>
        ...
        <div>{openIssues.slice(0, 5).map((issue) => <div className="flex items-center gap-3 border-b border-[#22282e] p-4 last:border-0" key={issue.id}>
          <span className={`grid size-8 shrink-0 place-items-center rounded-lg ${issue.severity === "High" ? "bg-[#2f1519] text-[#e87980]" : issue.severity === "Medium" ? "bg-[#2c210d] text-[#e0aa4b]" : "bg-[#17252e] text-[#79b9dc]"}`}><CircleAlert size={16}/></span>
        ...
        <Link className="flex items-center justify-between border-t border-[#272e34] p-4 text-xs font-semibold text-[#79b8d8] hover:bg-[#151a1e]" href="/management/variances">Review all exceptions <ChevronRight size={15}/></Link>
      </div>
    </div>
...
export function ProductProfitabilityScreen() {
  ...
  return <>
    ...
    <div className={`${card} overflow-hidden`}>
      <div className="flex flex-col gap-3 border-b border-[#272e34] p-4 md:flex-row md:items-center">
...
export function SupplierPerformanceScreen() {
  ...
  return <>
    ...
    <div className="mb-4 grid gap-4 lg:grid-cols-3">{suppliers.map((supplier, index) => <div className={`${card} relative overflow-hidden p-5`} key={supplier.supplier}>
      ...
      <div className="my-5 grid grid-cols-2 gap-3"><div className="rounded-lg bg-[#0d1013] p-3"><p className={label}>Avg carcass</p><p className="mt-2 font-mono font-semibold">{supplier.averageCarcassKg.toFixed(1)} kg</p></div><div className="rounded-lg bg-[#0d1013] p-3"><p className={label}>Avg cost</p><p className="mt-2 font-mono font-semibold">{zar.format(supplier.averageCostKg)}/kg</p></div></div>
      ...
      <div className="mt-6 space-y-5">
        {suppliers.map((supplier) => {
          ...
          return <div key={supplier.supplier}>
            ...
            <div className="h-3 overflow-hidden rounded-full bg-[#20262b]">
...
function VarianceRow({ issue, reviewed, onReview }: { issue: VarianceIssue; reviewed: boolean; onReview(issue: VarianceIssue): void }) {
  return <tr className="border-b border-[#20262b] text-xs last:border-0 hover:bg-[#151a1e]"><td className="px-5 py-4"><Status tone={issue.severity === "High" ? "red" : issue.severity === "Medium" ? "amber" : "blue"}>{issue.severity}</Status></td><td className="px-5 py-4">{issue.category}</td><td className="px-5 py-4 font-mono font-semibold text-[#acd2e5]">{issue.reference}</td><td className="px-5 py-4 text-[#7d8790]">{formatDate(issue.occurredAt)}</td><td className="max-w-xs px-5 py-4 text-[#9ca6ad]">{issue.description}</td><td className={`px-5 py-4 text-right font-mono ${issue.quantityKg && issue.quantityKg < 0 ? "text-[#e87980]" : "text-[#67d098]"}`}>{issue.quantityKg === undefined ? "—" : `${issue.quantityKg > 0 ? "+" : ""}${issue.quantityKg.toFixed(2)} kg`}</td><td className={`px-5 py-4 text-right font-mono ${issue.value < 0 ? "text-[#e87980]" : ""}`}>{zar.format(issue.value)}</td><td className="px-5 py-4 text-right">{reviewed ? <Status tone="green">Reviewed</Status> : <button className="rounded-md border border-[#3a4851] px-3 py-2 text-[10px] font-semibold text-[#88bad3] hover:bg-[#17252e]" onClick={() => onReview(issue)}>Review</button>}</td></tr>;
}
...
export function VariancesScreen() {
  ...
  return <>
    <Header eyebrow="Management / exception control" title="Variances & waste" copy="Investigate every stock, receiving, till, and waste exception; record management sign-off without deleting the source event.">
      <div className="flex gap-2"><select aria-label="Variance category" className={input} value={category} onChange={(event) => setCategory(event.target.value)}><option>All</option><option>Stock count</option><option>Waste</option><option>Till</option><option>Receiving</option></select><button className={`rounded-lg border px-3 text-xs ${showReviewed ? "border-[#31546a] bg-[#132735] text-[#79b9dc]" : "border-[#303840] text-[#8c969e]"}`} onClick={() => setShowReviewed((value) => !value)}>Include reviewed</button></div>
    </Header>
...
export function ReconciliationScreen() {
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.05fr_.95fr]">
      <div className={`${card} overflow-hidden`}>
        <div className="flex items-center justify-between border-b border-[#272e34] p-5"><div><h2 className="text-sm font-semibold">Ledger bridge</h2><p className="mt-1 text-xs text-[#6f7a84]">Opening + inflows − outflows ± adjustments</p></div><Status tone={result.reconciled ? "green" : "red"}>{result.reconciled ? "Balanced" : "Investigate"}</Status></div>
        <div className="p-5">{flows.map((flow, index) => <div className={`flex items-center gap-4 py-4 ${index < flows.length - 1 ? "border-b border-[#242a30]" : ""}`} key={flow.label}><span className={`grid size-9 place-items-center rounded-lg font-mono font-semibold ${flow.sign === "+" ? "bg-[#163a2a] text-[#61cd94]" : flow.sign === "−" ? "bg-[#2f1519] text-[#e87980]" : "bg-[#17252e] text-[#79b9dc]"}`}>{flow.sign || "="}</span><span className="flex-1 text-sm text-[#9ca6ad]">{flow.label}</span><strong className="font-mono">{flow.sign}{Math.abs(flow.value).toFixed(2)} kg</strong></div>)}</div>
        <div className="grid grid-cols-2 gap-px border-t border-[#272e34] bg-[#272e34]"><div className="bg-[#0d1013] p-5"><p className={label}>Theoretical closing</p><p className="mt-2 font-mono text-xl font-semibold">{result.expectedClosingKg.toFixed(2)} kg</p></div><div className="bg-[#0d1013] p-5"><p className={label}>Counted / system physical</p><p className="mt-2 font-mono text-xl font-semibold">{result.physicalClosingKg.toFixed(2)} kg</p></div></div>
      </div>
      <div className={`${card} p-5`}>
        ...
        <div className="mt-5 space-y-3 rounded-lg border border-[#293139] bg-[#0c0f12] p-4 text-xs"><div className="flex justify-between"><span className="text-[#7d8790]">Physical less expected</span><strong className={`font-mono ${result.reconciled ? "text-[#67d098]" : "text-[#e87980]"}`}>{result.varianceKg > 0 ? "+" : ""}{result.varianceKg.toFixed(3)} kg</strong></div><div className="flex justify-between"><span className="text-[#7d8790]">Value impact</span><strong className="font-mono">{zar.format(result.varianceValue)}</strong></div><div className="flex justify-between"><span className="text-[#7d8790]">Open butcher reservations</span><strong className="font-mono">{result.reservedKg.toFixed(3)} kg</strong></div></div>
